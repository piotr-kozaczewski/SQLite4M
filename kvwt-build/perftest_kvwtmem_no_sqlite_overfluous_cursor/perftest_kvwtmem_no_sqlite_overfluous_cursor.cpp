// perftest_kvwtmem_no_sqlite_overfluous_cursor.cpp : 
...
//        ***/ "perftest_kvwtmem_no_sqlite__overfluous_cursor" 
   ...
   virtual int do_work()
   {
      ...
      for (size_t i = 0; i < n_num_iter; ++i)
      {
         ...
         for (size_t j = 0; j < m_numrows_per_txn; ++j)
         {
            ...
            // overfluous cursor open
            sqlite4_kvcursor * pCursor = NULL;
            int rc = kvwtOpenCursor(m_pKVStore, &pCursor);
            if (rc != SQLITE4_OK)
            {
               printf("Cannot open cursor, failed kvwtOpenCursor(...): %s\n", sqlite4_errmsg(0));
               kvwtCloseCursor(pCursor);
               exit(-1);
            }
...
int main(int argc, char* argv[])
{
   ...
   if (argc == 5)
   {
      ...
      {
         printf("Usage:\nperftest_kvwtmem_no_sqlite_overfluous_cursor numrows_total numrows_per_txn numthreads lazy|eager\n");
         return -1;
      }
   }
   else
   ...
   printf("nperftest_kvwtmem_no_sqlite_overfluous_cursor   numrows_total=%d numrows_per_txn=%d numthreads=%d lazy_init=%s \n", 
