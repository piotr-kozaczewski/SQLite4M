...
#include "kvbdb_common.h" /* for Declaration/typing of the Dictionary Structure and global init routine */
...
u_int32_t nGlobalDefaultInitialCursorKeyBufferCapacity  = 16384; // 16 k
u_int32_t nGlobalDefaultInitialCursorDataBufferCapacity = 16384; // 16 k
...
{
   std::lock_guard<std::mutex> oGuard(s_bdb_dict_mutex);
...
int init_bdb_dict(bdb_dict_t ** a_bdb_dict_pp)
{
   if (*a_bdb_dict_pp == NULL)
   {
      ...
      bdb_dict_t * pNew = (bdb_dict_t *)new bdb_dict_t;
      if (!pNew)
      {
                    ...
                    char * node_name )
{
   ...
   a_bdb_dict_p->mutex.lock();
    ...
    if(pCurrDictNode)
    {
       ...
       pCurrDictNode->mutex.lock();

       // unlock dictionary
       a_bdb_dict_p->mutex.unlock();
    ...
    bdb_dict_node_t * pNew = (bdb_dict_node_t *) new bdb_dict_node_t;
    if(!pNew)
    {
...
int kvbdbClose(KVStore *pKVStore) {
   if ((KVBdb*)pKVStore)
   {
      //printf("-----> KVBdb::kvbdbClose() , database_name='%s'\n", ((KVBdb*)pKVStore)->name );
   }
   else
   {
      //printf("-----> KVBdb::kvbdbClose() , database_name='_none_'\n" );    
   }
